...
const char *password = "neeyati812";
const char* device_token  = "98c728b90b960097";
WiFiClient client;
//************************************************************************
String URL = "http://192.168.1.100/SchoolSecuritySystem/getSchoolGateData.php"; //computer IP or the server domain
int port = 80;
String getData, Link;
String OldCardID = "";
...
void loop() {
  ...
  //---------------------------------------------
  if (millis() - previousMillis >= 15000) { //Resets the old card every 15sec
  ...
  //---------------------------------------------
  if( CardID == OldCardID ){
    return;//go to the start of the loop if the same card was scanned within the last 15sec
  }
  else{
    OldCardID = CardID;
  }
